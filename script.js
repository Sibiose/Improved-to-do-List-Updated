...
function createTaskItem(event) {
  if (taskInput.value !== "" && event.keyCode === 13) {
    ...
    currentTheme === "light" ? newLi.classList.add("light-task-item") : null;
...
function updateList() {
  ...
  taskItemCompleted = taskItem.filter((taskItem) =>
    taskItem.classList.contains("completed-item")
  );

  taskItemActive = taskItem.filter(
...
function taskCountUpdate() {
  taskItem.length === 0
    ? (taskCount.innerText = "No task for today!")
    : (taskCount.innerText =
        taskItem.length - taskItemCompleted.length + " items left");
}
taskCountUpdate();
...
allFilter.addEventListener("click", function filterAll() {
  taskItem.forEach((item) => item.classList.remove("hide"));
});

activeFilter.addEventListener("click", function filterActive() {
  taskItemActive.forEach((item) => item.classList.remove("hide"));
  taskItemCompleted.forEach((item) => item.classList.add("hide"));
});

completedFilter.addEventListener("click", function filterCompleted() {
  taskItemCompleted.forEach((item) => item.classList.remove("hide"));
  taskItemActive.forEach((item) => item.classList.add("hide"));
});

const filters = [allFilter, activeFilter, completedFilter];
...
clearBtn.addEventListener("click", function clearAllCompleted() {
  taskItemCompleted.forEach((task) => task.remove());
});
...
toggleBtn.addEventListener("click", function toggleFun() {
  ...
  pseudoCheck.forEach((item) => item.classList.toggle("light-pseudo-check"));
  taskItem.forEach((item) => item.classList.toggle("light-task-item"));

  let sunSrc = "images/icon-sun.svg";
  let moonSrc = "images/icon-moon.svg";

  if (toggleImage.src.match(sunSrc)) {
    toggleImage.src = moonSrc;
    currentTheme = "light";
  } else {
    toggleImage.src = sunSrc;
    currentTheme = "dark";
  }
});
